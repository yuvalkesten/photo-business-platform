...
interface InstagramMessagesClientProps {
...
export function InstagramMessagesClient({
  ...
  const { data, isLoading, isFetching, error } = useMessagesPolling({
    classification,
    status,
    page,
  });
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-destructive">
          Error loading messages: {data?.error || "Unknown error"}
        </CardContent>
      </Card>
    );
  }
  ...
  const hasFailedMessages = messages?.some(
    (m) => m.status === "FAILED" || m.status === "PENDING"
  );
        ...
        <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
          ...
          {message.createdContact && (
            ...
            >
              ...
              {message.createdContact.firstName}{" "}
              {message.createdContact.lastName}
            </Link>
          )}
